    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex">
          ...
          {required && <p className="text-gray-500 font-bold">*</p>}
        </div>
        {hasEvent && (
          <div
      ...
      <div
        className={`flex w-80 items-center bg-gray-100 px-2 py-1 rounded-lg h-10 focus-within:bg-gray-50 ${className}`}
      >
        ...
        <input
          ...
          className={`bg-gray-100 text-gray-700  w-80 h-10 text-sm ml-1 px-1 focus:bg-gray-50 rounded-lg`}
        />
      </div>
      {isError && (
        <p className="text-gray-500 text-xs font-semibold mt-1">{meta.error}</p>
      )}
    </div>
  );
};
